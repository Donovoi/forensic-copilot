...
Catalog import uses a 64 KiB binary buffer on the already-held protected handle. It detaches without closing that handle and restores the consumed byte position before returning or raising, so read-ahead cannot alter subsequent verification. Exact raw bytes, offsets, hashes, explicit line limits, final unterminated rows and parse errors retain their existing meaning. Buffering does not change image access or whole-catalog coverage.
