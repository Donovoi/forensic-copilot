...
The examiner's first helper remains the senior tooling specialist. The prompt set includes platform profiling so the workflow separates evidence OS from runner OS before collection or OS-specific tool choice. After the senior handoff, OpenCode also supports scoped evidence collection, artifact routing, timeline analysis, attribution analysis, adversarial report challenge, publication redaction, and offline/no-download script fallback. Keep quick-triage prompts narrow; comprehensive prompts may ask the collector and router to preserve or inventory every relevant in-scope artifact class.

`forensic-attribution-analyst` is the internal device-user, owner/custodian, and public-source corroboration lane. Give it explicit fragment/query-log paths and online-research privacy controls. It returns a scoped attribution fragment for examiner review and must not edit or finalize the canonical report.
