...
- Do not run validation that modifies evidence, installs packages, downloads content, contacts a network, or prints secrets.
...
- Confirm read-only evidence handling, explicit args, fixed time windows, structured outputs, zero-row status files, safe secret handling, and complete logs.
