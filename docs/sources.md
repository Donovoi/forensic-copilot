...
| Microsoft Evaluation Center — Windows 11 Enterprise 25H2                              | current Windows evaluation media and the 90-day evaluation boundary           | Pins the Windows forensic VM base to current official Microsoft media rather than an unofficial forensic distribution. | <https://www.microsoft.com/en-us/evalcenter/evaluate-windows-11-enterprise> |
| Microsoft Windows 11 Enterprise Evaluation ISO hashes                                 | independent verification of the downloaded 25H2 x64 English ISO                | Provides the publisher SHA-256 that the VM-media download gate must match before customization or boot.  | <https://go.microsoft.com/fwlink/?linkid=2334901> |
| Microsoft Learn — Windows ADK and WinPE                                               | supported Windows 11 25H2 deployment and recovery environment                 | Supports building a network-isolated, case-specific WinPE repair environment instead of treating a generic VM as a forensic appliance. | <https://learn.microsoft.com/en-us/windows-hardware/get-started/adk-install>, <https://learn.microsoft.com/en-us/windows-hardware/manufacture/desktop/winpe-intro?view=windows-11> |
| Microsoft Learn — `chkdsk`                                                            | read-only status checks and explicit offline repair semantics                  | Keeps filesystem repair restricted to a separately hashed derivative and distinguishes inspection from `/offlinescanandfix`. | <https://learn.microsoft.com/en-us/windows-server/administration/windows-commands/chkdsk> |
...
| `Volatility 3`           | volatile-memory analysis and supported image layers              | primary open memory-analysis framework; test compatibility with the OS information plugin before broad collection   | <https://github.com/volatilityfoundation/volatility3> |
| Volatility ELF layer     | ELF64 segmented-memory mapping                                   | basis for attempting direct analysis of supported ELF64 physical-memory layouts                                     | <https://volatility3.readthedocs.io/en/latest/volatility3.framework.layers.elf.html> |
| QEMU memory dump docs    | QEMU guest-memory formats and Windows dump option                 | confirms ELF and Windows full-dump acquisition formats and the conversion context                                   | <https://www.qemu.org/docs/master/interop/qemu-qmp-ref.html> |
| QEMU `elf2dmp`           | QEMU ELF-to-Windows-crash-dump conversion                        | controlled fallback when a Windows QEMU ELF capture cannot be consumed directly                                    | <https://gitlab.com/qemu-project/qemu/-/tree/master/contrib/elf2dmp> |
...
| `Plaso` Docker docs      | supported container acquisition and volume-mapping path          | basis for using the upstream container rather than maintaining a duplicate local package stack                      | <https://plaso.readthedocs.io/en/latest/sources/user/Installing-with-docker.html> |
