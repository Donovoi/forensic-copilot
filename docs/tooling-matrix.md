...
For explicit recovered-media outputs, the Windows [metadata/header worker](recovered-media.md) binds approved producer records and separately provisioned ExifTool/ffprobe packages. It uses a fixed binary-signature format set, controlled child processes and verified capture reuse. Unknown formats, diagnostics and resource stops remain gaps; this is not full decoding or an authenticity assessment.
