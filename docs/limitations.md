...
The Windows [recovered-media worker](recovered-media.md) covers only explicit producer-bound exports and its fixed PNG/JPEG/WAVE/MP4-MOV signature set. Bounded metadata/header checks do not establish full decode, authenticity or accurate historical dates. Unknown types, partial recovery, external-track warnings, timeouts and resource stops remain explicit gaps. Polled storage thresholds and Windows job limits do not provide filesystem quotas or a native-parser security sandbox.
...
- Secret-bearing artifacts require controlled handling. Preserve, inventory, parse, or dump them with hashes and provenance when they are in scope; minimize exposure outside approved case outputs and report secret values only when the case specifically requires that disclosure.
- Some AI interfaces, provider policies, system instructions, or enterprise rules may prohibit plaintext secret handling. That should not remove secret extraction from the forensic workflow; it means the extraction lane must move to approved local tools, offline execution, or a local model, with only redacted summaries returning to restricted interfaces.
- Extracted secrets can unlock additional evidence, but they do not automatically expand legal authority or technical scope. Local in-scope unlock attempts should be logged; remote, cloud, third-party, or otherwise scope-expanding use should be brought back to the user unless it is already explicitly authorized.
