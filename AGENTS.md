...
- Select tools by current capability and validation, not a fixed product: follow `docs/tool-selection-and-validation.md`, compare credible alternatives, record a primary method and independent checks for material results, and preserve unresolved disagreements. NIST catalog inclusion is not tool approval.
...
- Treat extracted secrets as evidence leads. Classify the secret type, source artifact, likely program/site/service, local or remote use, confidence, controlled output path, and next allowed action; use local in-scope secrets to unlock more evidence, and ask before remote or scope-expanding use.
