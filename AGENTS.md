...
- Keep OpenCode local-model helper outputs bounded: researcher notes should be 20 lines or fewer, provisioner notes 25 lines or fewer, and senior handoffs 30 lines or fewer. The senior must call the provisioner immediately after the researcher returns; focused helper requests should return directly instead of spending extra turns on todo lists or interim prose.
...
- Keep `AGENTS.opencode.md` and `docs/opencode-agents/` as the lean OpenCode runtime policy and prompt set. `AGENTS.md` and `.github/agents/` remain the canonical full policy and Copilot-compatible prompts, but local-model OpenCode runs should not load the full repo policy into every helper request.
...
- In WSL OpenCode live-host triage, avoid raw PowerShell `$` tokens inside double-quoted `powershell.exe -NoProfile -Command` strings. Use fixed literal timestamps and simplified syntax such as `Where-Object StartTime -GE [datetime]'YYYY-MM-DDTHH:MM:SS'`, or escape `$` deliberately when a variable is unavoidable.
