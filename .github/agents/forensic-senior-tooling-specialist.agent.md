...
```json
{
  ...
  "prompt": "Offline Windows last-2h activity. Generate smallest read-only script with args, logs, zero-row status, controlled secret mode if in scope. Do not run. Return <=12 lines."
}
```
