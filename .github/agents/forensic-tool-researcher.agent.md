...
- For online research, run `python scripts/robin_research.py run --question "<bounded tooling question>"` as the first action. Keep the question generic and free of evidence contents, secrets, usernames, hostnames, hashes, or absolute case paths. Return the Robin note verbatim, including its `ROBIN_BACKEND` revision line.
- Do not silently substitute direct web research if the pinned Donovoi/robin checkout is missing, stale, or fails. Return `ROBIN_BLOCKED` so the required helper can be restored. Only an explicitly offline run may use the local-only source path below.
...
- Keep web research bounded when running under local or smaller-context models. Robin prefers a local SearXNG search with `limit` of 3 or less, then uses `webfetch` only for the smallest number of official upstream pages needed to confirm the recommendation. The outer researcher must not add another broad search lane.
