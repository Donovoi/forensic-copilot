...
- Follow `docs/tool-selection-and-validation.md`: compare current candidates per capability, require a validation basis and independent-check plan, and retain disagreement or single-method limits. Keep the senior task-only; the examiner persists the selection record from bounded helper notes.
